...
func RegistryPath() (string, error) {
	...
	return filepath.Join(home, ".barista", "gradle.json"), nil
}
...
func Load(path string) (*Registry, *output.ErrInfo) {
	...
	for i := range reg.Installations {
		...
		if e.Name == "" {
			return nil, configError("%s: gradle entry missing required field \"name\"", filepath.ToSlash(path))
		}
		if seen[e.Name] {
			return nil, configError("%s: duplicate gradle name %q", filepath.ToSlash(path), e.Name)
		}
		seen[e.Name] = true
		if e.Version == "" {
			return nil, configError("%s: gradle %q missing required field \"version\"", filepath.ToSlash(path), e.Name)
		}
		if e.Path == "" {
			return nil, configError("%s: gradle %q missing required field \"path\"", filepath.ToSlash(path), e.Name)
		}
	}
	return reg, nil
}
...
func (r *Registry) Save(path string) *output.ErrInfo {
	...
	tmp, err := os.CreateTemp(dir, "gradle.json.*")
	if err != nil {
...
func (r *Registry) Add(e Entry) *output.ErrInfo {
	if r.Find(e.Name) != nil {
		return &output.ErrInfo{
			...
			Message: fmt.Sprintf("gradle %q is already registered", e.Name),
			Hint:    "choose another name or run: barista gradle remove " + e.Name,
		}
	}
...
func (r *Registry) Remove(name string) (*Entry, bool, *output.ErrInfo) {
	...
	return nil, false, &output.ErrInfo{
		...
		Message: fmt.Sprintf("gradle %q is not registered", name),
	}
}
