...
func TestParse(t *testing.T) {
	...
	for _, tc := range cases {
		segs, q, err := Parse(tc.in)
		if tc.wantErr {
			if err == nil {
				t.Errorf("Parse(%q): want error", tc.in)
			}
			continue
		}
		if err != nil || !reflect.DeepEqual(segs, tc.segs) || q != tc.qualifier {
			t.Errorf("Parse(%q) = %v, %q, %v; want %v, %q", tc.in, segs, q, err, tc.segs, tc.qualifier)
		}
	}
}
...
func TestLooksLike(t *testing.T) {
	...
	for in, want := range cases {
		if got := LooksLike(in); got != want {
			t.Errorf("LooksLike(%q) = %v, want %v", in, got, want)
		}
	}
}
