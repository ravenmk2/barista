...
func addCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "add <path>",
		Short: "Register an installed Node.js (runs node --version to read the version)",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			*exitCode = 0
			...
			if e := reg.Add(node.Entry{Name: name, Version: info.Version, Path: info.Home}); e != nil {
				failRes(e)
				return nil
			}
	...
	cmd.Flags().String("name", "", "register under this name (default: node-<version> from the probed version)")
	cmd.Flags().Bool("default", false, "set as the default node")
	return cmd
}
