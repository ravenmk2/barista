...
func uninstallCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short:             "Delete a barista-installed Node.js from disk and unregister it",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			*exitCode = 0
			...
			if !entry.Managed {
				failRes(&output.ErrInfo{
					Code:    output.CodeNodeNotManaged,
					Message: fmt.Sprintf("node %q was not installed by barista", name),
					Hint:    "to unregister without deleting files, run: barista node remove " + name,
				})
				return nil
			}
			root, err := node.InstallDir(reg.InstallDir)
			if err != nil {
			...
			if err != nil || rel == ".." || strings.HasPrefix(rel, ".."+string(filepath.Separator)) {
				failRes(&output.ErrInfo{
					...
					Hint:    "fix node.json manually or run: barista node remove " + name,
				})
				return nil
			}
			...
			if err := os.RemoveAll(managedDir); err != nil {
				failRes(&output.ErrInfo{Code: output.CodeNodeUninstallFailed, Message: err.Error()})
				return nil
			}
