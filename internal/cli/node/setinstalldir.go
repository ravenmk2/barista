...
func setInstallDirCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Set the managed Node.js install root (node.json installDir); --reset restores the builtin default",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			*exitCode = 0
			...
			if reset, _ := cmd.Flags().GetBool("reset"); reset {
				...
				def, err := node.DefaultInstallDir()
				if err != nil {
	...
	cmd.Flags().Bool("reset", false, "clear installDir and restore the builtin default (~/.barista/toolchains/node)")
	return cmd
}
