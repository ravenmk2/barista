...
func setInstallDirCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Set the managed Maven install root (maven.json installDir); --reset restores the builtin default",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			*exitCode = 0
			...
			if reset, _ := cmd.Flags().GetBool("reset"); reset {
				...
				def, err := maven.DefaultInstallDir()
				if err != nil {
	...
	cmd.Flags().Bool("reset", false, "clear installDir and restore the builtin default (~/.barista/toolchains/maven)")
	return cmd
}
