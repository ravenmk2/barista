...
func configCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "config",
		Short: "Show the effective gradle configuration (default, jdk, installDir) with its source",
		Args:  cobra.NoArgs,
			...
			*exitCode = 0
			...
			if wsRoot := findWorkspaceRoot(); wsRoot != "" {
				...
				for _, name := range []string{"init.gradle", "init.gradle.kts"} {
					s := filepath.Join(wsRoot, ".barista", "gradle", name)
					if _, err := os.Stat(s); err == nil {
						scripts = append(scripts, filepath.ToSlash(s))
					}
				}
				if len(scripts) > 0 {
					detail["init"] = map[string]any{"value": scripts, "source": "workspace"}
				} else {
					detail["init"] = map[string]any{"value": "", "source": "ambient"}
				}
				if v, ok := props.String("gradle.user.home"); ok && v != "" {
					v = workspace.ExpandHome(v)
					if !filepath.IsAbs(v) {
						v = filepath.Join(wsRoot, v)
					}
					detail["gradleUserHome"] = map[string]any{"value": filepath.ToSlash(v), "source": "workspace"}
				} else {
					detail["gradleUserHome"] = map[string]any{"value": "", "source": "none"}
				}
			}
			...
			if eff.Default == "" {
				...
				if e := reg.Find(eff.Default); e != nil {
					...
					d["error"] = map[string]any{
						...
						"message": fmt.Sprintf("default gradle %q is not registered", eff.Default),
					}
				}
				detail["default"] = d
			}
			...
			installDir, err := gradle.InstallDir(reg.InstallDir)
			if err != nil {
				fail(cmd, &output.ErrInfo{Code: output.CodeConfigError, Message: err.Error()})
				return nil
			}
...
func printConfig(detail map[string]any, p output.Palette) {
	...
	_, _ = fmt.Fprintf(w, "default\t%s\t%s\n", defValue, sourceLabel(def, "gradle.json"))
	...
	_, _ = fmt.Fprintf(w, "jdk\t%s\t%s\n", jdkValue, sourceLabel(jdkD, "gradle.json"))
	...
	if sd, ok := detail["init"].(map[string]any); ok {
		src := "-"
		var v string
		switch scripts := sd["value"].(type) {
		case []string:
			v = strings.Join(scripts, ", ")
			src = "workspace (.barista/gradle/init.gradle[.kts])"
		case string:
			v = p.Dim("(none, Gradle default applies)")
		}
		_, _ = fmt.Fprintf(w, "init\t%s\t%s\n", v, src)
	}
	if ld, ok := detail["gradleUserHome"].(map[string]any); ok {
		v, _ := ld["value"].(string)
		if v == "" {
			v = p.Dim("(none)")
		}
		_, _ = fmt.Fprintf(w, "gradle.user.home\t%s\t%s\n", v, sourceLabel(ld, "properties.json"))
	}

	inst := detail["installDir"].(map[string]any)
	_, _ = fmt.Fprintf(w, "installDir\t%v\t%s\n", inst["value"], sourceLabel(inst, "gradle.json"))
	_ = w.Flush()
}
