...
func listCmd() *cobra.Command {
	return &cobra.Command{
		Use:     "list",
		Aliases: []string{"ls"},
		Short:   "List registered Gradle installations",
		Args:    cobra.NoArgs,
...
func printList(reg *gradle.Registry, eff effectiveSettings, p output.Palette) {
	if len(reg.Installations) == 0 {
		fmt.Println(p.Dim("no Gradle installations registered"))
		return
	}
