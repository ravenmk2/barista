...
func discoverCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "discover",
		Short: "Discover installed Gradles (GRADLE_HOME env, sdkman, brew, scoop, system locations, PATH) and register them",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			*exitCode = 0
			...
			cands := gradle.DiscoverCandidates()
			if len(cands) == 0 {
				if !jsonOut {
					fmt.Println("no Gradle candidates found")
				}
			...
			for _, c := range cands {
				...
				if e := reg.Add(gradle.Entry{Name: name, Version: info.Version, Path: info.Home}); e != nil {
...
func registeredAs(reg *gradle.Registry, home string) string {
