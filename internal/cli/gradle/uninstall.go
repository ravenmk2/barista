...
func uninstallCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short:             "Delete a barista-installed Gradle from disk and unregister it",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			*exitCode = 0
			...
			if !entry.Managed {
				failRes(&output.ErrInfo{
					Code:    output.CodeGradleNotManaged,
					Message: fmt.Sprintf("gradle %q was not installed by barista", name),
					Hint:    "to unregister without deleting files, run: barista gradle remove " + name,
				})
				return nil
			}
			root, err := gradle.InstallDir(reg.InstallDir)
			if err != nil {
			...
			if err != nil || rel == ".." || strings.HasPrefix(rel, ".."+string(filepath.Separator)) {
				failRes(&output.ErrInfo{
					...
					Hint:    "fix gradle.json manually or run: barista gradle remove " + name,
				})
				return nil
			}
			...
			if err := os.RemoveAll(managedDir); err != nil {
				failRes(&output.ErrInfo{Code: output.CodeGradleUninstallFailed, Message: err.Error()})
				return nil
			}
