...
func whichCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		ValidArgsFunction: comp.Fn(comp.GradleSpecs),
		Short:             "Resolve a Gradle installation by name, version (progressively widened), or the effective default",
	...
	cmd.Flags().Bool("pathonly", false, "print only the gradle home path, without a trailing newline")
	return cmd
}
...
func pathCmd() *cobra.Command {
	return &cobra.Command{
		...
		ValidArgsFunction: comp.Fn(comp.GradleSpecs),
		Short:             "Print the resolved gradle home path (shortcut for: which --pathonly)",
		Args:              cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func homeCmd() *cobra.Command {
	return &cobra.Command{
		...
		ValidArgsFunction: comp.Fn(comp.GradleSpecs),
		Short:             "Print the resolved gradle home path (same as: path)",
		Args:              cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			runResolve(cmd, firstArg(args), true)
			return nil
