...
func setInstallDirCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Set the managed JDK install root (jdk.json installDir); --reset restores the builtin default",
	...
	cmd.Flags().Bool("reset", false, "clear installDir and restore the builtin default (~/.barista/toolchains/jdk)")
	return cmd
}
