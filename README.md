...
- **JDK / Maven / Gradle 工具链管理**：发现、注册、一键安装（Temurin / Microsoft JDK / Corretto / Zulu / GraalVM 等 JDK 发行版）、多版本默认切换
- **工作区感知的 mvn / java / gradle**：按 repo / 工作区 / 用户多级自动选用 JDK 与构建工具版本，注入私有 settings / init script 与本地仓库
- **环境体检与自更新**：`barista doctor` 诊断配置与检出，`barista upgrade` 一键升级
- **动态 shell 补全**：bash / zsh / fish / PowerShell，仓库名、JDK、Maven / Gradle 等参数智能候选
