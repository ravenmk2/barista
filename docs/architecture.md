...
internal/
  ...
  workspace/        工作区发现（向上找 .barista/，FindWorkspaceRoot 含 home 守卫）、repos.json 与两级 config.json 加载合并、properties.json 读写、cwd→repo 匹配（MatchRepo 最长前缀）
