...
- 下载镜像（config `download.mirror` / `maven.download.mirror` 或 `--mirror` flag 显式覆盖，详见 [mirror.md](mirror.md)）：配置了镜像时把官方 URL 的 `DistBase`（`https://archive.apache.org/dist`）前缀替换为镜像基址（`MirrorArchiveURL`，路径模板不变）作为主下载源，官方 URL 作回退（`WithFallback`，回退时 stderr 提示完整官方 URL）；sha512 校验值恒从官方 `.sha512` 取；JSON detail 带 `downloadUrl`（实际使用 URL）与 `mirror`（配置的原始值）
