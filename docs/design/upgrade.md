...
3. 下载平台 asset：归档文件（windows 为 zip，其余为 tar.gz；包内二进制条目为 `barista` / `barista.exe`，另有 LICENSE 与 completions 等附带文件）；复用 download 包（断点续传 + 重试 + TTY 进度条，重试次数同其他下载命令，`--attempts` 可覆盖）
4. sha256 校验归档，不符报 UPGRADE_CHECKSUM_MISMATCH
5. 按 manifest 的 `format` / `entry` 从归档提取二进制（不支持的 format 报 UPGRADE_EXTRACT_FAILED 并提示手动安装）
6. 替换 `os.Executable()`：
...
- release workflow 调用 `python3 scripts/genmanifest.py` 生成 schemaVersion 2 清单：每个 asset 记录 file / format（zip | tar.gz）/ entry（包内二进制路径）/ sha256 / size，sha256 与 size 针对归档文件本身；workflow 另产出独立的 checksums.txt，不被 manifest 消费
- 归档命名 `barista-<version>-<goos>-<goarch>.(zip|tar.gz)`，包内二进制统一为 `barista` / `barista.exe`，附带 LICENSE 与 `completions/`（bash/zsh/fish/powershell，由 cobra completion 命令在构建时生成）
- v2 相对 v1 不兼容：旧客户端（要求 schemaVersion 1）读新清单会干净报错而非装坏文件；v2 内的新增可选字段靠 `additionalProperties: true` 保持向后兼容
