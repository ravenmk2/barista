...
> 全局契约见 [../architecture.md](../architecture.md)；本文档是下载镜像机制（跨 jdk / maven / gradle / node 四域的 install / download 路径）的详细设计，改动相关代码前必读。

## 背景与目标

官方下载源（Adoptium API → GitHub Releases、archive.apache.org、services.gradle.org、nodejs.org）在国内网络下慢或不可达。本机制允许为四个域分别配置二进制下载镜像，其余链路不变。

两条贯穿性原则：

- **镜像只替换二进制下载基址，元数据恒走官方**：版本列表（Adoptium API、archive.apache.org 目录、`services.gradle.org/versions/all`、`nodejs.org/dist/index.json`）与校验和（maven sha512、gradle sha256、node SHASUMS256.txt、zulu/graalvm 内嵌钉值）的来源不因镜像配置改变。
...
| 键 | 值 | 作用域 |
| --- | --- | --- |
| `download.mirror` | 全局预设名（见下） | 一键给四个域赋默认映射 |
...
| `node.download.mirror` | `official` / 预设名 / 自定义基址 URL | node install |

优先级（高→低）：下载命令的 `--mirror` flag（显式给出时覆盖一切 config，`--mirror official` 临时禁用镜像）> 分域键 > 全局 `download.mirror` > `official`。flag 值域与对应 config 键一致（jdk 仅预设名，maven / gradle / node 另接受自定义基址），非法值在参数校验阶段报用法错误（exit 2）。

### 预设

预设名是"每域一个基址"的映射表，不是单一镜像站（没有一家镜像站同时覆盖四个域）：

| 预设 | jdk（仅 temurin） | maven | gradle | node |
| --- | --- | --- | --- | --- |
| `cn` | TUNA Adoptium | TUNA apache | 腾讯云 | npmmirror |
| `tuna` | TUNA Adoptium | TUNA apache | —（TUNA 无 gradle，设为 official） | TUNA nodejs-release |
| `huawei` | —（华为无 Temurin，设为 official） | 华为云 apache | 华为云 gradle | 华为云 nodejs |
| `tencent` | —（腾讯无 Temurin，设为 official） | 腾讯云 apache | 腾讯云 gradle | —（腾讯云无 node，设为 official） |

分域键取 `tuna` 之类单站预设时按上表取对应列；该列无覆盖的域保持 `official`。新增预设只需在映射表加行，配置层无感知。

### 自定义基址

仅 maven / gradle / node 接受自定义 URL（镜像都是纯 base 替换，路径模板与官方一致）：
...
| node | `https://nodejs.org/dist` | `https://mirrors.tuna.tsinghua.edu.cn/nodejs-release` | — | `https://repo.huaweicloud.com/nodejs` | `https://cdn.npmmirror.com/binaries/node` |
...
- node：基址替换后拼 `v<version>/node-v<version>-<suffix>`（suffix 如 `win-x64.zip` / `linux-arm64.tar.gz`），与官方 dist 布局一致（三个预设镜像均为同构 dist 布局）
...
- install 的归档下载走镜像；`index.json` 元数据与 `SHASUMS256.txt` 校验和仍从官方取
- 三个预设镜像（npmmirror / TUNA nodejs-release / 华为云 nodejs）均为同构 dist 布局；404 回退行为与 maven 一致
...
- 离线测试沿用 httptest 可变基址（gradle 的 `VersionsBase`、node 的 `DistBase` 模式）：镜像基址映射表做成数据，测试注入自定义预设或自定义基址即可覆盖
- 不新增命令与 flag；配置通过手工编辑 config.json + `barista schema show config` / `schema validate` 自描述
- `doctor` 不主动探测镜像可达性（不被动联网）；镜像故障由下载时的回退契约兜住

## 改动面清单

- `schemas/config.schema.json`：新增镜像键及 description（含"元数据恒走官方"说明）
- `internal/workspace`：config 结构加镜像字段（user + workspace 合并）
- `internal/jdk` / `internal/maven` / `internal/gradle` / `internal/node`：install/download 的 URL 解析接入镜像映射（预设表 + 自定义基址校验）
